...
BOOL IsISearchExploreChar(int meta, int key) {
    return meta == 0 && ((key >= 'A' && key <= 'Z') || (key >= '0' && key <= '9') || (key >= 'a' && key <= 'z') || key == '_' || key == '-' || key == '.');
}
...
static BOOL is_isearch_char(int key) {
    return key == 9 || (key >= ' ' && key <= '~') && key != '\\' || key >= 128;
}
...
static BOOL match_back(int start) {
    ...
    if (string_c_str(gInputFileName)[0] != 0) {
        ...
        for (i = start; i >= 0; i--) {
            ...
            if (!gISearchPartMatch && result == fnamev || gISearchPartMatch && result != NULL) {
    ...
    if (string_length(gInputFileName) <= 2) {
        ...
        for (i = start; i >= 0; i--) {
            ...
            if (all_english) {
                ...
                if (!gISearchPartMatch && r2 == 0 || gISearchPartMatch && r2 >= 0) {
                    (void)filer_cursor_move(adir(), i);
                    return TRUE;
                }
            }
