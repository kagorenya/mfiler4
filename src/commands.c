...
BOOL cmd_keycommand(sObject *nextin, sObject *nextout, sRunInfo *runinfo) {
    if (runinfo->mBlocksNum == 1 && runinfo->mArgsNumRuntime == 2) {
        ...
        int meta = 0;
        if (sRunInfo_option(runinfo, "-m")) {
            meta = 1;
        }

        filer_add_keycommand(meta, keycode, runinfo->mBlocks[0], "run time script",
                             external);
    ...
    if (runinfo->mArgsNumRuntime == 2) {
        ...
        if (strcmp(arg1, "+") == 0) {
            ...
            if (!filer_add_history(dir, arg1)) {
                err_msg("adding hisotry of mcd failed", runinfo->mSName,
                        runinfo->mSLine);
                return FALSE;
            }
...
BOOL cmd_file_name(sObject *nextin, sObject *nextout, sRunInfo *runinfo) {
    if (mis_raw_mode() && nextout == gStdout) {
        ...
        if (file) {
            if (!fd_write(nextout, string_c_str(file->mName),
                          string_length(file->mName))) {
                err_msg("interrupt", runinfo->mSName, runinfo->mSLine);
                runinfo->mRCode = RCODE_SIGNAL_INTERRUPT;
                return FALSE;
            }
...
BOOL cmd_path(sObject *nextin, sObject *nextout, sRunInfo *runinfo) {
    if (mis_raw_mode() && nextout == gStdout) {
        ...
        sDir *dir2 = filer_dir(dir);
        if (dir2) {
            if (!fd_write(nextout, string_c_str(dir2->mPath),
                          string_length(dir2->mPath))) {
                err_msg("interrupt", runinfo->mSName, runinfo->mSLine);
                runinfo->mRCode = RCODE_SIGNAL_INTERRUPT;
                return FALSE;
            }
...
BOOL cmd_file_user(sObject *nextin, sObject *nextout, sRunInfo *runinfo) {
    if (mis_raw_mode() && nextout == gStdout) {
        ...
        if (file) {
            if (!fd_write(nextout, string_c_str(file->mUser),
                          string_length(file->mUser))) {
                err_msg("interrupt", runinfo->mSName, runinfo->mSLine);
                runinfo->mRCode = RCODE_SIGNAL_INTERRUPT;
                return FALSE;
            }
...
BOOL cmd_file_group(sObject *nextin, sObject *nextout, sRunInfo *runinfo) {
    if (mis_raw_mode() && nextout == gStdout) {
        ...
        if (file) {
            if (!fd_write(nextout, string_c_str(file->mGroup),
                          string_length(file->mGroup))) {
                err_msg("interrupt", runinfo->mSName, runinfo->mSLine);
                runinfo->mRCode = RCODE_SIGNAL_INTERRUPT;
                return FALSE;
            }
...
static void cmdline_start(char *cmdline, int cursor, BOOL quick,
                          BOOL continue_) {
    def_prog_mode();
    endwin();
  ...
  */
    ...
    if (continue_) {
        ...
        if (!xyzsh_readline_interface_onetime(&rcode, cmdline, cursor, "cmdline",
                                              NULL, 0, NULL)) {
            rcode = -1;
        }
    }
    ...
    if (rcode == 0 || rcode == -2) {
        ...
        int rcode;
        (void)xyzsh_eval(&rcode, buf, "reread", NULL, gStdin, gStdout, 0, NULL,
                         gMFiler4);
    }
        ...
        /// 状態セット ///
            ...
            if (dir < 0) {
                ...
                if (reread) {
                    ...
                    int rcode;
                    (void)xyzsh_eval(&rcode, buf, "reread", NULL, gStdin, gStdout, 0,
                                     NULL, gMFiler4);
                    //(void)filer_reread(0);
                    //(void)filer_reread(1);
                }
...
static void draw_progress_box(int mark_num) {
    ...
    mvprintw(y + 1, (maxx - 22) / 2 + 1, "(%d/%d) files", gProgressMark,
             mark_num);
}

// when returning TRUE, log is opened. you need to call finish_logging or
// fclose(log);
            ...
            for (j = 0; j < mark_file_num; j++) {
                ...
                if (strcmp(fname, ".") != 0 && strcmp(fname, "..") != 0) {
                    ...
                    if ((num = filer_file2(adir(), fname)) < 0) {
                        ...
                        if (!copy_file(source, destination, FALSE, preserve, &override_way,
                                       &remove_write_protected, log, &err_num)) {
                            break;
                        }
            ...
            for (j = 0; j < mark_file_num; j++) {
                ...
                if (strcmp(fname, ".") != 0 && strcmp(fname, "..") != 0) {
                    ...
                    if ((num = filer_file2(adir(), fname)) < 0) {
                        ...
                        if (!copy_file(source, destination, TRUE, preserve, &override_way,
                                       &remove_write_protected, log, &err_num)) {
                            break;
                        }
...
BOOL cmd_mrm(sObject *nextin, sObject *nextout, sRunInfo *runinfo) {
    ...
    if (!mis_raw_mode()) {
        err_msg("invalid terminal setting. not raw mode", runinfo->mSName,
                runinfo->mSLine);
...
BOOL cmd_mln(sObject *nextin, sObject *nextout, sRunInfo *runinfo) {
    ...
    if (runinfo->mArgsNumRuntime == 2) {
        ...
        if (stat(destination, &dstat) < 0 || !S_ISDIR(dstat.st_mode)) {
            err_msg("mcp: destination is not directory", runinfo->mSName,
                    runinfo->mSLine);
            xendwin();
            return FALSE;
        }
                ...
                if (strcmp(fname, ".") != 0 && strcmp(fname, "..") != 0) {
                    ...
                    if ((num = filer_file2(adir(), fname)) < 0) {
                        ...
                        if (symlink(source, dfile) < 0) {
                            merr_msg("can't make symlink(%s). Is it attempt to make on vfat?",
                                     dfile);
                            break;
                        }
...
BOOL cmd_kanjicode_file_name(sObject *nextin, sObject *nextout,
                             sRunInfo *runinfo) {
...
BOOL cmd_mclear_immediately(sObject *nextin, sObject *nextout,
                            sRunInfo *runinfo) {
    if (!mis_raw_mode()) {
        err_msg("invalid terminal setting. not raw mode", runinfo->mSName,
                runinfo->mSLine);
...
void commands_init() {
    ...
    xyzsh_add_inner_command(gMFiler4, "mclear_immediately",
                            cmd_mclear_immediately, 0);
    ...
    xyzsh_add_inner_command(gMFiler4, "kanjicode_file_name",
                            cmd_kanjicode_file_name, 0);
