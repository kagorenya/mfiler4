...
BOOL filer_cursor_move(int dir, int num) {
    ...
    if (strcmp(env_view_option, "2pane") == 0 || strcmp(env_view_option, "1pane") == 0) {
...
void str_cut2(enum eKanjiCode code, char *mbs, int termsize, char *dest_mbs, int dest_byte) {
    if (code == kUtf8) {
        ...
        wchar_t *wcs = (wchar_t *)MALLOC(sizeof(wchar_t) * (termsize + 1) * MB_CUR_MAX);
        wchar_t *tmp = (wchar_t *)MALLOC(sizeof(wchar_t) * (termsize + 1) * MB_CUR_MAX);
...
static void make_file_stat(sFile *file, char *buf, int buf_size) {
    ...
    if (env_mtime && strcmp(env_mtime, "1") == 0) {
        ...
        snprintf(buf + strlen(buf), buf_size - strlen(buf), " %02d-%02d-%02d %02d:%02d", year, tm_->tm_mon + 1, tm_->tm_mday, tm_->tm_hour, tm_->tm_min);
    }
...
static void make_file_name(sFile *file, char *fname, int fname_size, char *ext, int ext_size, char *dot, int name_len) {
    ...
    if (S_ISDIR(file->mStat.st_mode)) {
        ...
        xstrncat(fname, "=", fname_size);
    } else if (env_add_star_exe && strcmp(env_add_star_exe, "1") == 0 && (file->mStat.st_mode & S_IXUSR || file->mStat.st_mode & S_IXGRP || file->mStat.st_mode & S_IXGRP)) {
        xstrncat(fname, "*", fname_size);
...
void filer_view(int dir) {
    ...
    if (strcmp(env_view_option, "1pane") == 0) {
        if (self->mActive) {
            ...
            for (i = self->mScrollTop;
                 i - self->mScrollTop < maxy - kMaxYMinus - dstack_exist - jobs_exist && i < vector_count(self->mFiles);
                 i++) {
                int attr = 0;
                ...
                } else if (file->mStat.st_mode & S_IXUSR || file->mStat.st_mode & S_IXGRP || file->mStat.st_mode & S_IXGRP) {
            ...
            if (page_view_way) {
                ...
                mvprintw(maxy - kMaxYMinus + 1 - jobs_exist, 2, "(%d/%d)-%sbytes-(%3d%%)", filer_mark_file_num(dir), vector_count(self->mFiles) - 1, size, p);
            } else {
                if (filer_line_max(adir()) > 0) {
                    ...
                    mvprintw(maxy - kMaxYMinus + 1 - jobs_exist, 2, "(%d/%d)-%sbytes-(%d/%d)", filer_mark_file_num(dir), vector_count(self->mFiles) - 1, size, cur_page, max_page);
                }
            }
        }
    }
    ...
    /////////////////////////////////////////////
        ...
        if (self->mActive) {
            ...
            int i;
            for (i = self->mScrollTop;
                 i - self->mScrollTop < (maxy - kMaxYMinus - dstack_exist - jobs_exist) * 2 && i < vector_count(self->mFiles);
                 i++) {
                int attr = 0;
                sFile *file = (sFile *)vector_item(self->mFiles, i);
                ...
                const int x = (i - self->mScrollTop) / (maxy - kMaxYMinus - dstack_exist - jobs_exist) * (maxx / 2) + 1;
                const int y = (i - self->mScrollTop) % (maxy - kMaxYMinus - dstack_exist - jobs_exist) + 1 + dstack_exist;
    ...
    //////////////////////////////////////////////
        ...
        if (self->mActive) {
            ...
            int i;
            for (i = self->mScrollTop;
                 i - self->mScrollTop < (maxy - kMaxYMinus - dstack_exist - jobs_exist) * 3 && i < vector_count(self->mFiles);
                 i++) {
                int attr = 0;
                sFile *file = (sFile *)vector_item(self->mFiles, i);
                ...
                const int x = (i - self->mScrollTop) / (maxy - kMaxYMinus - dstack_exist - jobs_exist) * (maxx / 3) + 1;
                const int y = (i - self->mScrollTop) % (maxy - kMaxYMinus - dstack_exist - jobs_exist) + 1 + dstack_exist;
    ...
    //////////////////////////////////////////////
        ...
        if (self->mActive) {
            ...
            int i;
            for (i = self->mScrollTop;
                 i - self->mScrollTop < (maxy - kMaxYMinus - dstack_exist - jobs_exist) * 5 && i < vector_count(self->mFiles);
                 i++) {
                int attr = 0;
                sFile *file = (sFile *)vector_item(self->mFiles, i);
                ...
                attron(attr);
                const int x = (i - self->mScrollTop) / (maxy - kMaxYMinus - dstack_exist - jobs_exist) * (maxx / 5) + 1;
                const int y = (i - self->mScrollTop) % (maxy - kMaxYMinus - dstack_exist - jobs_exist) + 1 + dstack_exist;
                if (file->mMark) {
                    mvprintw(y, x, "*");
                } else {
    ...
    //////////////////////////////////////////////
        ...
        mvprintw(maxy - kMaxYMinus + 1 - jobs_exist, x + 2, "(%d/%d)-%sbytes", filer_mark_file_num(dir), vector_count(self->mFiles) - 1, size);
...
void cmdline_view_filer() {
    ...
    if (!S_ISLNK(file->mLStat.st_mode)) {
        ...
        snprintf(buf2, 1024,
                 ...
                 ,
                 permission, file->mLStat.st_nlink, owner, group

                 ,
                 size_buf2

                 ,
                 year, tm_->tm_mon + 1, tm_->tm_mday, tm_->tm_hour, tm_->tm_min, string_c_str(file->mNameView));
        ...
        snprintf(buf, 1024,
                 ...
                 ,
                 tm_->tm_year - 100, tm_->tm_mon + 1, tm_->tm_mday, tm_->tm_hour, tm_->tm_min, string_c_str(file->mNameView), string_c_str(file->mLinkTo));
...
int filer_line(int dir) {
    ...
    if (dir2) {
        if (strcmp(env_view_option, "2pane") == 0) {
            ...
            return (dir2->mCursor - dir2->mScrollTop) % (maxy - kMaxYMinus - dstack_exist - jobs_exist);
        }
    }
...
int filer_row_max(int dir) {
    ...
    if (dir2) {
        if (strcmp(env_view_option, "2pane") == 0) {
            ...
            return (vector_count(dir2->mFiles) - 1) / (maxy - kMaxYMinus - dstack_exist - jobs_exist) + 1;
        }
    }
