...
void sDir_setmask(sDir *self, char *mask) {
    ...
    int r = onig_new(&self->mMaskReg, regex, regex + strlen((char *)regex), ONIG_OPTION_IGNORECASE, ONIG_ENCODING_UTF8, ONIG_SYNTAX_DEFAULT, &err_info);
...
static BOOL sort_name(void *left, void *right) {
    ...
    if (dir_up && strcmp(dir_up, "1") == 0) {
        if (!S_ISDIR(l->mStat.st_mode) && !S_ISDIR(r->mStat.st_mode) || S_ISDIR(l->mStat.st_mode) && S_ISDIR(r->mStat.st_mode)) {
